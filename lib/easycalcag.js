...
$(window).load(function() {
	...
	setTimeout(function() {
		...
		// updateDisplayData()
		// fullCalc();
		// $('.bxslider').find('img[alt="PODOFEET MAX"]').eq(0).click();
		...
		setTimeout(function() {
			...
			// $('.bxslider').find('img[alt="PODOCLEAN"]').eq(0).click();
...
function priceEventsInit() {
	...
	.on('focus', function() {
		if (oldValue) {$(this).data('oldValue',$(this).val());}
	})
...
function findProductSliders(productname,duration) {
	...
	$('#slider-container').find('.sliderImg').each(function() {
		...
		if ($(this).isVisible() && $(this).find('img')[0].alt==productname && !$('.cat').eq(catIndex).hasClass('oldcat') && !$('.cat').eq(catIndex).hasClass('notactive') && !$('.cat').eq(catIndex).find('img[alt="'+productname+'"]').length && catIndex!=lastCat) {
			cats.push(catIndex);
		}
	});
...
function removeLastChars(myString,cars) {

	myString=myString.substring(0,myString.length -cars);
...
function appendSliderItems() {
	...
	"#controllers nocalculations",
		"KERSIA FOAMER","QUICK SPRAY SYSTEM","PREFOAM SYSTEM","PERFO DOSE","KERSIA WATER SYSTEM","KIT PETIT RUMINANT",
...
function productDatabaseSetup() {
	...
	prodText=[
		...
		"LIQ-IO 2500", 								"#Udder Film", 		"3,7", 															"",			"kg,kg,kg,kg",								"1,Dip-io%202500%20FR.pdf?csf=1&e=jI055R", 						"LIQ-IO-2500", 					"Liq-io 2500 est un désinfectant contenant 2500 ppm d'iode, enrichie en substances adoucissantes, émollientes et hydratantes|||",																																																																																																											"4x5,10,22,60,120,200,1000",	"",			"kg,kg,kg,kg",									"hygiène mamelle*Iode,udder hygiene*Iodine,,",								"",		"manual",			"",										"",										"cow",						"012801,012805,012807,012808,012806,012804,012803",				"AB",	"B",
		...
		"HYPRASIL MAÏS+",							"#Valor",			"1",															"",			"dose(s),dose(s),Dosen,dosis",				"0,ER_EmBWmMydPu6YCrw5YZWkB2GjwSm8kRYC3NZtrzfyeOA?e=XAuPJw",	"HYPRASIL-MAÏS+",				"Valorisateur pour ensilage de maïs (maïs grain humide et maïs épis) ● Contient des bactéries homo et hétéro fermentaires ● Assure une plus grande stabilité du fourrage dans le temps ● 1 dose = un sachet de 100g pour 100 Tonnes brutes|||",																																																																																"1",				"",						"sachet(s),bag(s),,",							"valorisation,valorization,,",												"",		"auto",				"",										"",										"cow,goat,sheep",			"",																"",		"",
