...
var App='Easycalc',
	...
	loadError='clear',
	saveError='clear',
	subCatClassNames=['subconsforcalc','subconsforcalc2','consforcalc','unitprice','boxpackagingNum','boxpackagingVal'],
...
$(window).load(function() {
	...
	setTimeout(function() {
		...
		// $('.bxslider').find('img[alt="HYPRASIL GREEN+"]').eq(1).click();
		// $('.bxslider').find('img[alt="HYPRASIL MAÏS+"]').eq(0).click();
		// $('.bxslider').find('img[alt="HYPRASIL DUO"]').eq(0).click();
		// $('.bxslider').find('img[alt="BOLIFAST RUMEN"]').eq(0).click();
		// $('#viewswitcher').click()
		setTimeout(function() {
			// $('.bxslider').find('img[alt="HYPRASIL DUO"]').eq(0).click();
...
function saveProductImgDB() {
	...
	if (!$('body').data('cors-Img-OK')) {console.log('Souci CORS, changez de navigateur !');return;}
	...
	for (var i=6; i<prodText.length; i+=prodText.length/prodDataNumb) {
		...
		$(img).on('load', function() {
			var srcImg=$(this).attr('src').replace(imgFolder+SL,N).replace(smallImgExt,N);
...
function mainEventInit() {
	...
	.on('click','.modal-eraser', function() {
		$(this).closest('.modal').find('input').val(N);
		$(this).closest('.modal').find('select').val('blank');
		filesStatusUpdate();
	})
...
function showmodal(target,special) {
	...
	switch(modalName) {
		...
		case 'valorDetail': {
			...
			.text(target.closest('.catbox').find('.imgbox').attr('alt')+SP+CL+SP+modalVFText[lang+(special-1)*langScope]);
			break;
		}
	}
...
function diagEngine(action) {
	...
	// Removed 22-07-2019 (not wanted anymore by Kersia)
	// Logo presence //
...
function createPriceList() {
	...
	for (var i=0; i<prodText.length; i+=prodText.length/prodDataNumb) {
		...
		if (prodText[i]!=hideProduct && prodText[i+14]!='NAP' && prodText[i+12]!='old') {
			...
			htmlX+='<img class="priceBox-img tooltipelement" src="'+imgFolder+SL+prodText[i+6]+'" data-title="'+prodText[i+7]+'" alt="'+prodText[i]+'">';
...
function priceEventsInit() {
	...
	.on('blur', function(e) {
		var myYear = $(this).val();
		if (!myYear) {$(this).val(dataYear).trigger('keyup');}
...
function createProdMosaic(mosType) {
	...
	if (!create) {
		$('.catbox, #categories').find('img').each(function() {
			allSelectedItems+=$(this).attr('alt')+CI;
		});
	} else {
...
function createListFromRange(myRange) {
	...
	$.each(theRange, function(index,value) {
		...
		$('.mosaicProdBox-img').each(function() {
			...
			if (myProdImg.attr('alt').indexOf(nameCreate)>=0) {
...
function animalMenu() {
	...
	disableMouseWheel();
	hideRightMenu(1);
	$('#backtotop').addClass('notdisplayed');
	...
	.off().on('click', function(e) {
		...
		animalType=$(this).find('.animal-image').attr('src').split(DH)[1].split(PO)[0];
...
function hideAnimalMenu(nochange) {
	...
	$('#backtotop').removeClass('notdisplayed');
...
function findProductSliders(productname,duration) {
	...
	var cats=[], catIndex, lastCat=catText.length/langScope, htmlX=N;
	...
	$('#slider-container').find('.sliderImg').each(function() {
		...
		if ($(this).isVisible() && $(this).find('img').attr('alt')==productname && !$('.cat').eq(catIndex).hasClass('notactive') && !$('.cat').eq(catIndex).find('img[alt="'+productname+'"]').length && catIndex!=lastCat) {
			cats.push(catIndex);
		}
	});
	...
	if (cats.length) {
		$.each(cats, function(index,value) {
			htmlX+='<div class="catCart-container fasttransition tooltipelement" data-cat="'+value+'" data-title="'+specialWindowsText[lang+16*langScope]+'"><div class="catCart-icon"/><div class="cartCart-name">'+catText[1+lang+(langScope+1)*value].split(LI)[0]+ED+ED;
			// htmlX+=clearBoth;
		});
		$('#lightbox-cart').show();
...
$.fn.addProduct=function() {
	...
	if (product.siblings('.bxcheck').isVisible() && iscatbox && (!catalog || theCat.hasClass('controllers'))) {
		box.each(function(index) {
			...
			if (productname==searchArticle.attr('alt')) {
	...
	// Show '✓' and remove '+'
	product
	.siblings('.bxplus').hide()
	.siblings('.bxcheck').show();
	...
	if (Optionshippingbottom=='true' && productname!='DELIVERY' && endOfCart.prev().find('.imgbox').attr('alt')=='DELIVERY') {endOfCart=endOfCart.prev();}
	endOfCart
	.before('<div class="catbox lastadded"><img class="imgbox lightboxenabled" src="#"><div class="productboxname"/><span class="SAPCode"/><div class="productboxinfo-container"><label class="specialCalc"><input type="checkbox" class="specialCalcCheck"><div class="nicecheckbox"/><span/></label><div class="productboxshortdesc"/></div><div class="close"/><div class="magnify"/><div class="box-biocide"/></div>');
...
function newboxTexts() {
	...
	if (specials!='NAP') { // NAP is for Not A Product (E.g. DELIVERY)
		...
		for (var i=0; i<packaging.length; i++) {
			classPack='packaging';
			if (i===0 && demoProcess && (productname.indexOf('TRAYOR')>0 || productname.indexOf('ACTIFLASH')>0)) {
	...
	if (biocideM) {
		...
		.text(biocideText[$.inArray(country,countries)]).end()
		.show();
	}
	...
	.append('<div class="consinfo"/><div class="subconsforcalc-container"><input tabindex='+(tabIndex-1)+' class="subconsforcalc adjust" value="0"><div class="prodcalc"></div><input tabindex='+(tabIndex-1)+' class="subconsforcalc2 adjust" value="0"><div class="prodcalc2"/><div class="subconsforcalc3 mediumtransition"><div class="consforcalcbefore"/><div class="consforcalcafter"/><div class="consforcalcmode"/><div class="lactoduc">Lactoduc</div><div class="hoovesProtocol"/><div class="valorDetail"/></div><div class="moreconsinfo"/></div><div class="separate">'+clearBoth+ED+htmlX)
	...
	.find('.consinfo').css('color',catcolor).end()
...
function shortenText(target,maxLength) {
	...
	while (target.length>maxLength) {
		target=target.substr(0,target.lastIndexOf(CI))+'[…]';
	}
...
$.fn.removeProduct=function() {
		...
		.animate({
			...
			sliderArticle.find('img').each(function() {
				var nameImg=$(this).attr('alt'),
					scannedArticle=$(this).parent();
				if (nameImg==boxToremove.find('img').attr('alt')) { 
					$(this)
...
function boxCalculations(box) {
	...
	.find('.forfree span').text(freeText[lang]);
...
function circuitsCalc(box) {
	...
	var boxImg=box.find('img'),
		attrib=findAttributesInArray(animalType,animalAttributes),
		waterPerCluster=attrib[4],
		//isCow=(animalType=='cow'),
		milkingDays=attrib[2]/2; // (milkings/(2 milkings per day))
...
function tankCalc(box) {
	...
	subconsforcalc=box.find('.subconsforcalc').val();
	subconsforcalc2=box.find('.subconsforcalc2').val();
	if (Number(subconsforcalc2)===0) {box.find('.subconsforcalc2').val(Math.ceil(cleanings));}
	subconsforcalc2=box.find('.subconsforcalc2').val();
	prodcalc=parseInt(subconsforcalc*0.01*watercons*density*subconsforcalc2);
	prodcalcText='% x &W x &D x'
	.replace('&W',watercons)
...
function pawCalc(box) {
	...
	box.find('.subconsforcalc2, .subconsforcalc3, .prodcalc2, .specialCalc, .hoovesProtocol').show()
	box.find('.subconsforcalc-container').addClass('showOnHover');
	...
	subconsforcalc=box.find('.subconsforcalc').val();
	if (Number(subconsforcalc)===0) {
		box.find('.subconsforcalc').val(5);
	}
	subconsforcalc2=box.find('.subconsforcalc2').val();
	if (Number(subconsforcalc2)===0) {
		box.find('.subconsforcalc2').val(4);
	}
	...
	if (box.find('.specialCalcCheck').prop('checked')) {
		...
		$('.consforcalc , .subconsforcalc-container, .monthsforbox, .packarrow, .multiply, .consforcalcText').hide();
		box.data('selectType','nocalc');
		onDemandCalc(box);
		return
	} else {
		box.data('selectType','estimate');
		$('.consforcalc, .subconsforcalc-container, .monthsforbox, .packarrow, .multiply, .consforcalcText').show();
	}
	...
	prodcalc=parseInt(subconsforcalc*consinfo*subconsforcalc2/100);
...
function waterCalc(box) {
	...
	if (productname=='GERMICIDAN TABS' || productname=='AQUASEPT') {
		...
		if (productname=="Lessive de soude" || productname=="ANTI-GERM ACID'O") {onDemandCalc(box,1);return;}
...
function simpleCalc(box) {
	...
	if (productname=="Extrait de Javel") {onDemandCalc(box,1);return;}
	...
	if (catType.indexOf('controllers')>=0) {
...
function onDemandCalc(box,force) {
...
function choosePackagingInit() {
	...
	$(document).on('click','.packaging', function(e) {
		...
		// Is a price set?
		var findPrice=scanPriceFile(box,boxImg.attr('alt'),boxpackagingVal);
...
function adjustInputInit() {
	...
	.on('change','.adjust', function() {
		...
		if (isNaN(correctedInput)) {correctedInput='';} // Case modal table
	...
	.on('blur','.adjust', function(e) { // Default value if no value entered
		...
		validateEstimate(0);
		estimateEnable();
		if (!$(this).val() || ($(this).hasClass('discount') && $(this).val()>100)) {$(this).val(this.defaultValue);}
...
function doseCountInit() {
	...
	$(document).on('change', 'input.tableCell', function() {
		...
		for (var i=1; i<=3; i++) {
			...
			raw=modal.find('.dosing').text().match(/\d+/);

			if (!raw) {raw=raws[$(this).attr('data-row')-2];}

			// Do the calculation only when not divided by 0 and result is not 0
			if (raw*tmsP) {
				result=Math.round(((surf*tms)/(tmsP/100))/raw);
				doses.val(result ? result : '');
			}
...
function updateCatTotal(cat) {
	...
	if (!cat.find('.cattotalprice').text()) {
		...
		.next().text(totalsumText[lang]+CL);
	}
...
function rightmenuShowNumbers() {
	...
	$('#rightmenu-container').find('.rightmenuli').each(function() {
		...
		if (theNumb) {
			$(this).append('<div class="rightmenunumbs" style="left:'+$(this).find('span').width()+'px;">'+theNumb+'</div>');
		}
	});
...
function modalInit() {
	...
	var modals=['#modalVF1,1600,1','#modalVF2,700,2','#modalVF3,1600,3'];
	...
	$.each(modals, function(index, value) {
		...
		createModalVFTable(value[0],value[1],value[2]);
	});
	doseCountInit();
...
function createModalVFTable(target, dim, type) {
	...
	var myDiv=$(target),
		...
		addClass2,
		htmlX='',
		rowInfos=[1,0,1], // This is where (index) the row info begins
	...
	$.each(myArray, function(row, value) {
		...
		$.each(cells, function(col, value) {
			...
			if (value) {
				content=valorText ? (valorText.split(CO)[lang] ? valorText.split(CO)[lang] : valorText.split(CO)[0]) : '';
				// Prevents text overflow
				if (content.length>11*dimDiv/86) {
					addClass=SP+'font11'
					if (content.length>17*dimDiv/86) {
						addClass+=SP+'centeredInput';
					}
				}
				htmlX+='<div class="tableCell' + SP + value + addClass + '" data-row="'+row+'" data-col="'+col+'">'+content+'</div>';				
			} else {
				// htmlX+='<select class="tableCell' + SP + addClass + '" data-row="'+row+'" data-col="'+col+'">'+value+'</option><option value="saab">Saab</option><option value="mercedes">Mercedes</option></select>';
	...
	// Create option list
	$.each(valorHarvestTest, function(index, value) {
		value=value.split(CO)[lang];
	...
	myDiv.find('input').each(function() {
		...
		inputClass=inputClass.replace('tableCell ','').replace('font11','').replace(' centeredInput','');
...
function mergeCells(target) {

	var myDiv=$(target),
		myRefCell={}, lastMergeCell,
		removed=0;
...
function createStructure() {
	...
	htmlX+='<div id="modalHP" class="modal modal-hoovesProtocol"><img src="'+imgFolder+SL+'hooves-protocol-fr.png"></div>'; // An image is set to define <w> & <h>
	...
	$('#main-container')
	.after('<div id="topWidget-container"><div id="topWidget"/></div><div id="small-data-widget"/><div id="backtotop"/>')
...
function createPriceFile() {
	...
	.each(function() {
		...
		inputPrice.each(function() {
			price=$(this).val() || 'x';
			// pack=Number($(this).prev().text().split(SP)[0]) || 1;
	...
	$('.cat','#categories').each(function() {
		...
		if (!currCatBox) {
			...
			catScan.find('.catbox').each(function() {
				...
				currCatBox=$(this);
				currInfo+=CR+currCatBox.find('img').attr('alt')+CL;
...
function updateEstimate() {
	...
	.after('<div class="sheetfooter"><div class="sheetpage"/><img src="'+imgFolder+'/logo-kersia.png" class="sheetlogofooterAG"><img src="'+DistriblogoSrc+'" class="sheetlogofooterDistrib"><div class="sheetbiocide">'+'(*)'+SP+biocideText[lang]+ED+ED);
	...
	$('.cat','#categories').each(function(k) {
		...
		if (sheetCat.find('.catbox').length) {
			...
			if ($(this).hasClass('catalogue')) {sheetArticleTitle=sheetText[lang+langScope*23];}
			sheetCatText=catText[1+lang+(langScope+1)*k].split(LI)[1] || catText[1+lang+(langScope+1)*k];
			$('.estimateSheet').eq($('.sheetpage').length-2)
			.append('<div class="sheettitlecat">'+sheetCatText+ED);

			htmlX='<div class="sheetproductBox"><div class="sheetBoxTitle">'+sheetArticleTitle+ED+ED;
			htmlX+='<div class="sheetcalcUPBox"><div class="sheetBoxTitle">'+sheetText[9*langScope+lang]+ED+ED;
			htmlX+='<div class="sheetcostperanimBox"><div class="sheetBoxTitle">'+sheetText[19*langScope+lang].replace('&S8',animalInLang)+ED+ED;
			htmlX+='<div class="sheetproductBox"><div class="sheetBoxTitle periodTitle">'+sheetText[8*langScope+lang].replace('&S7',$('#months').text())+ED+ED;
			...
			sheetCat.find('.catbox').each(function() {
				...
				columns=$('.estimateSheet').find('.sheettitlecat').eq($('.sheettitlecat').length-1).nextAll();

				// Biocidal product? * -> *Biocidal phrases
				var sourceProdName=$(this).find('img').attr('alt');
				biocideAS='';
				...
				columns.eq(1)
				.append('<div class="sheetproduct">'+detailCons+ED);
				...
				if (!Number($(this).data('discountedPrice')) || parseInt(sheetBoxPrice)==0) {
					...
					if (parseInt(sheetBoxPrice)==0) {sheetBoxDiscountSum=-1;}
				}
				...
				.next()
				.append('<div class="sheetproduct">'+sheetBoxCons+ED)
				.next()
				.append('<div class="sheetcalcprice">'+sheetBoxUnitPrice+ED)
				.next()
				.append('<div class="sheetcalcprice">'+sheetBoxPrice+ED)
				.next()
				.append('<div class="sheetcalcprice '+discountClass+'">'+sheetBoxDiscount+ED)
				.next()
	...
	targetColumns=$('.discounted, .nodiscount').parent();
	if (!sheetBoxDiscountSum) {
	...
	targetColumns.each(function() {
		if ($(this).text().indexOf('%')<0 && $(this).text().indexOf(freeText)<0) {$(this).children().html(NBSP);}
	});
	...
	// Just memorize the raw value
	var discount=sheetBoxDiscountSum;
	...
	for (var i=0; i<nbsheetProducts; i++) {
		for (var j=0; j<prodText.length; j+=prodText.length/prodDataNumb) {
			position=prodText[j].indexOf(realProducts.eq(i).attr('data-productname'));
			// Checks also if product already there
			if (position>=0) {
				if ($('.sheetpicname'+indexCss).text().indexOf(prodText[j])>=0) {
		...
		if (readyToGo) {
			...
			html2canvas(sheetToPDF.eq(page), {
				...
				whenCreated:function(canvas) {
					...
					if (OptioncostGMilk=='true') {swapPDFimage('.sheetcminfo img',0,sheetToPDF.eq(page),imgCCInfo,dimRatio);}
					if (OptioncostGDCM=='true') {swapPDFimage('.sheetccinfo img',0,sheetToPDF.eq(page),imgCCInfo,dimRatio);}
					...
					if (page==maxPage) {
						...
						pdf.setDisplayMode('100%',null); // 100% zoom (instead of full page) + thumbs displayes
...
function recreateProductImg(sheetToPDF,page,dimRatio) {
	...
	if (!$('body').data('cors-Img-OK')) {
		console.log('Création des images produits !');
		$('.sheetpic1, .sheetpic2').each(function(index) {

			var productDB,
				imgName,
				imgNameSRC=$(this).css('visibility','hidden').attr('src').replace(imgFolder+SL,N).replace(smallImgExt,N),
				imgData,
...
function lightbox() {
	...
	.on('click','.lightboxenabled', function() {
		...
		if ($(this).attr('data-title')) {
			...
			position=$.inArray($(this).attr('alt'),prodText);
			...
			// Set attributes
			$('#lightbox-title').html($(this).attr('alt')).removeClass('lightbox-logoname');
			...
			$('#lightbox-desc').addClass('notdisplayed');
...
function setElementsColors() {
	...
	$('.selectproduct','#categories').each(function(index) {
		...
		.find('.bx-caption-app').css('color',catcolor);
	});
...
function topWidgetBounce(delay,duration) {
	...
	setTimeout(function() {
		...
		.animate({
			...
			$('#backtotop').css('right',30); // To prevent these elements from flashing on start
			$('#small-data-widget').css('left',30); // To prevent these elements from flashing on start
...
function showHideSliderInit() {
	...
	$('#open-slider').on('mouseover', function() {
		clearTimeout(timer3);
		hideRightMenu();
		showSlider();
	});
...
$.fn.autoAdjustInputs=function() {
	...
	$(this).find('.consforcalc, .subconsforcalc, .subconsforcalc2, .unitprice').autoGrowInputHor();
...
function setDemoSequence() {
	...
	// or
	// .made,200,write:AGRIMATDUPONT,… 		------ write 'AGRIMATDUPONT' on .made input/textarea
	// or
	// #div,200,textwrite:AGRIMATDUPONT,… 	------ write 'AGRIMATDUPONT' on #div
	// or
...
function appendCategories() {
	...
	$('.slider','#slider-container').each(function(index) {
		addAClass = (catText[(langScope+1)*index]=='old') ? SP+'hidden oldcat' : '';
...
function appendDataValues(index) {
	...
	databoxValue[1]=[
		...
		1,300,1000,1				// Animals 			(min, default, max, increment)
	];
...
function appendSliderItems() {
	...
	"#udderbeforem notSheepCat",
		"PROXYLAV","TRAYLAV","TRAYOR","LIQ-IO 2500","LIQ-IO C","PREMOUSS NET","HYPRODERM","NATIGREEN","HYPRED QUICK SPRAY","PREFOAM +","G-MIX","WIPES LSA","KIT SPRAY!goatsheep","ANTI-GERM FRESH","STERIPIS","PRATIC!cow","STERITRAITE","SEPTIFLASH","OUATE","TRAY'CLEAN","LAVETTES",
	"#clusters notSheepCat",
		"PERFO GRIF","ACTIFLASH","DES OXI-25",
	"#udderafterm",
		"HM VIR SPRAY","DIP BLUE LT","PROPISDERM","DIP-IO","LIQ-IO 2500","LIQ-IO 5500","LIQ-IO C","GOLDEN MIX","G-MIX","HY-COSMETIC","FILMADINE","HM VIR FILM","HM VIR SPRAY","HYPRED QUICK SPRAY","DERMIODE","POWER BLUE MIX","NOVODIP","TRAYMIX","TRAYOR","KIT SPRAY!goatsheep","TRAYDOU","TRAYFILM","TRAYDIP","IO-SPRAY","IO-FILM","IOFILM","PRATIC","MAMOGEL","INTEGRAL","SEPTIFLASH",
	"#betweenmilkings notGoatCat notSheepCat",
		"HYPRA'ZUR","DERMISAN +","PROXYLAV",
	"#robotic notGoatCat notSheepCat",
		"ROBOSPRAY IODE","ROBOSPRAY LACTIC","HM VIR SPRAY","ROBOSPRAY SUPREME","LIQ-IO 2500","LIQ-IO 5500","LIQ-IO C","QUICK SPRAY",
	...
	"#milktank",
		...
		"ADIROX","PENNGAR L.30","PENNGAR NPH","ACIDOGAL","CHLOROGAL","AOC-ADM","REMINOX","REMILIN","MAXIGAL","DES OXI-25",
		"GALORAN","PENNGAR L.20","PENNGAR L.X.","EXOPENNGAR",	"#surfaces",
	...
	"#accessories",
		"LAVETTES","GOBELET","BACTOSPRAY","PULVÉRISATEUR","KIT SPRAY!goatsheep","CM-TEST","DÉVIDOIR","SEAU","Kit bandelettes","CDN","LESSIVE","Extrait de Javel","POMPE",
	...
	for (var i=0; i<productRange.length; i++) {
		...
		for (var j=0; j<prodText.length; j+=prodText.length/prodDataNumb) {
			...
			if (position>=0 || (catalog && prodText[j]!=hideProduct)) {
				...
				if (prodNo<2 && (prodText[j].indexOf('TRAYOR')>0 || prodText[j].indexOf('ACTIFLASH')>0)) {
		...
		if (productRange[i][0]==HA) {
			catSlider++;
			htmlX+='<div class="slider slidertransition" data-type="'+productRange[i].substring(1)+'"><ul class="bxslider">';
			nbProd=0;
		} else {
			if (productRange[i+1]) {
				// Close the slider
				if (productRange[i+1][0]==HA) {
					$('#categories').find('.titlecat').eq(catSlider).attr('data-title',nbProd+SP+productText[(nbProd>1)*1].split(CO)[lang]);
					htmlX+='</ul>'+ED;
				}
			} else {
				// Close the slider
...
function specialCaptions() {
	...
	$('#slider-container').find('.sliderImg').each(function() {
		if ($(this).find('img').attr('alt')=='DELIVERY') {
...
function appendTexts() {
	...
	$('.forfree span').text(freeText[lang]);
	...
	$('#biocide').text(bioM); // country dependent
...
function textDatabaseSetup() {
	...
	valorTable=[];
	valorTable[1]=[
		"notvisible,notvisible,notvisible,cut1 cut,merge,merge,merge,merge,cut2 cut,merge,merge,merge,merge,cut3 cut,merge,merge,merge,merge",
		"dosing1 dosing,averageTMS,VFsurfaces,surface surface1,harvest harvest1,yield yield1,TMSha TMSha1,dosenb dosenb1,surface surface2,harvest harvest2,yield yield2,TMSha TMSha2,dosenb dosenb2,surface surface3,harvest harvest3,yield yield3,TMSha TMSha3,dosenb dosenb3",
		"meadow,,,,,,,,,,,,,,,,,",
		"catch,,,,,,,,,,,,",
		"meslin,,,,,,,",	
	]
	...
	valorTable[2]=[
		"dosing2 dosing,averageTMS,VFsurfaces,surface surface1,harvest harvest1,yield yield1,TMSha TMSha1,dosenb dosenb1",
		"cornsilage,,,,,,,",
		"earsilage,,,,,,,",
		"wetgrainsilage,,,,,,,"		
	]
	...
	valorTable[3]=[
		"notvisible,notvisible,notvisible,cut1 cut,merge,merge,merge,merge,cut2 cut,merge,merge,merge,merge,cut3 cut,merge,merge,merge,merge",
		"notvisible,averageTMS,VFsurfaces,surface surface1,harvest harvest1,yield yield1,TMSha TMSha1,dosenb dosenb1,surface surface2,harvest harvest2,yield yield2,TMSha TMSha2,dosenb dosenb2,surface surface3,harvest harvest3,yield yield3,TMSha TMSha3,dosenb dosenb3",
		"meadow,,,,,,,,,,,,,,,,,",
		"catch,,,,,,,,,,,,",
		"meslin,,,,,,,",
		"cornsilage,,,,,,,",
	...
	valorTableText['cut1']=				"1re coupe,1st cut,,";
	valorTableText['cut2']=				"2nde coupe,2nd cut,,";
	valorTableText['cut3']=				"3ème coupe,3rd cut,,";
	...
	optiondetailsText.push([
		...
		// "distriblogo,all,yes,enabled,Mon logo,My logo,Mein Logo,Mi logo",
	...
	pawinfoText=[
		"Saisir volume pédiluve et nbre de passages",
		"Please enter footbath vol and crossings",
		"Geben Sie Fussbad vol und Kreuzungen",
		"Introduzca footbath vol y cruces"
	];
	pawinfoTextSpray=[
		"Dosage : &C",
		"Dosing: &C",
	...
	warningText=[
		...
		"!Merci de vérifier votre saisie. Le format de l'adresse email que vous avez saisie n'est pas valide.",
		...
		"!Merci de vérifier votre saisie. Le format du numéro de téléphone que vous avez saisi n'est pas valide.",
	...
	catText=[
		...
		"act","Catalogue|Plus…","Catalogue|Plus…","Katalog|Mehr…","Catálogo|Más…"
	];
...
function productDatabaseSetup() {
	...
	//
	//Products: Name 								Type 				Consumption cow*goat*sheep										Robot 		Unit (nd is for not divisible)				Documents 														Image filename					Designation 																																																																																																																																								Packaging 						density 	Packaging unit									Application 											New 	Calculation mode 	Specials 								youTube 								Animals 					SAP 															AB 		Biocide	
	...
	prodText=[
		...
		"NATIGREEN",								"#Udder des",		"3,7,1.8,3",													"",			"kg,kg,kg,kg",								"", 															"NATIGREEN",					"Désinfectant à base d'acide lactique ● Prêt à l'emploi, agit en 30 sec ● Il contient aussi des tensio-actifs pour le nettoyage du trayon et de la glycérine pour l'assouplissement de la peau ● À appliquer en mousse ou en pulvérisation, puis essuyer à l'aide d'un papier à usage unique ou d'une lavette|||",																																																															"20,60",						"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"AOC(after)",							"",										"cow,goat,sheep",			"026312,026313",												"AB",	"",
		"PREFOAM +",								"#Udder des",		",,1.8,3",														"",			"kg,kg,kg,kg",								"1,Prefoam%20+%20fr.pdf?csf=1&e=GmNQjE", 						"PREFOAM-+",					"Désinfectant à base du complexe LSA® et de tensio-actifs, PREFOAM + est prêt à l'emploi ● Sa mousse est adhérente au trayon et s'essuie facilement à l'aide d'un papier à usage unique ou d'une lavette ● Laisse la peau du trayon propre, hydratée et assouplie|||",																																																																										"2x2,10,20,60,200,1000",		"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"AOC(after)",							"",										"cow,goat,sheep",			"011660,002753,002762,002778,002773,011228",					"AB",	"B",
		"DIP BLUE LT ACTIV'",						"#Udder Des",		"3",															"",			"kg,kg,kg,kg",								"",																"DIP-BLUE-LT-ACTIV'",			"Produit à base de dioxyde de chlore et d'acide lactique, pour la désinfection des trayons par trempage après la traite ● Efficacité prouvée pendant 8 jours après le mélange ● S'utilise en mélange 50/50 avec le DIP BLUE LT BASE ● Enrichi en glycérine, le produit marque le trayon d'une couleur bleue|||",																																																															"23,62",						"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"016586,016587",												"",		"B",
		"DIP BLUE LT BASE",							"#Udder Des",		"3",															"",			"kg,kg,kg,kg",								"",																"DIP-BLUE-LT-BASE",				"Produit à base de dioxyde de chlore et d'acide lactique, pour la désinfection des trayons par trempage après la traite ● Efficacité prouvée pendant 8 jours après le mélange ● S'utilise en mélange 50/50 avec le DIP BLUE LT ACTIV' ● Enrichi en glycérine, le produit marque le trayon d'une couleur bleue|||",																																																															"22,58",						"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"016591,016588",												"",		"B",
		"PROPISDERM",								"#Udder Des",		"3",															"",			"kg,kg,kg,kg",								"",																"PROPISDERM",					"Désinfectant à base de chlorhexidine, PROPISDERM est un produit de trempage épais prêt à l'emploi ● Il s'utilise sur le trayon après la traite et est enrichi en substances adoucissantes|||",																																																																																												"10,22,220",					"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"003138,003144,003152",											"AB",	"B",
		"GOLDEN MIX ACTIV'",						"#Udder Des",		"3",															"",			"kg,kg,kg,kg",								"",																"GOLDEN-MIX-ACTIV'",			"Produit à base de dioxyde de chlore et d'acide lactique, pour la désinfection des trayons après la traite ● S'utilise en mélange 50/50 avec le GOLDEN MIX BASE par trempage ● Enrichi en glycérine, coloration jaune sur le trayon|||",																																																																																	"2,10,23,62,229,1050",			"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"004929,004940,004954,004974,009175,005499",					"",		"B",
		"GOLDEN MIX BASE",							"#Udder Des",		"3",															"",			"kg,kg,kg,kg",								"",																"GOLDEN-MIX-BASE",				"Produit à base de dioxyde de chlore et d'acide lactique, pour la désinfection des trayons après la traite ● S'utilise en mélange 50/50 avec le GOLDEN MIX ACTIV' par trempage ● Enrichi en glycérine, coloration jaune sur le trayon|||",																																																																																	"2,10,22,58,214,980",			"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"005518,005529,005544,005564,009176,005568",					"",		"B",
		"G-MIX POWER ACTIV'",						"#Udder des",		"3,7,1.8,3|3,7,1.8,3",											"",			"kg,kg,kg,kg",								"", 															"G-MIX-POWER-ACTIV'",			"Désinfectant avant et après la traite à base de dioxyde de chlore, à utiliser en mélange 50/50 avec G-MIX POWER BASE ● Application en mousse ou pulvérisation ● Enrichi en tensio-actif pour le nettoyage, glycérine et acide lactique pour leurs propriétés cosmétiques|||",																																																																								"21,62,228",					"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"009423,009424,010215",											"AB",	"B",
		"G-MIX POWER BASE",							"#Udder des",		"3,7,1.8,3|3,7,1.8,3",											"",			"kg,kg,kg,kg",								"", 															"G-MIX-POWER-BASE",				"Désinfectant avant et après la traite à base de dioxyde de chlore, à utiliser en mélange 50/50 avec G-MIX POWER ACTIV' ● Application en mousse ou pulvérisation ● Enrichi en tensio-actif pour le nettoyage, glycérine et acide lactique pour leurs propriétés cosmétiques|||",																																																																							"19,58,212",					"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"009425,009426,010214",											"AB",	"B",
		"DIP-IO 2500", 								"#Udder Film", 		"3", 															"",			"kg,kg,kg,kg",								"1,Dip-io%202500%20FR.pdf?csf=1&e=jI055R", 						"DIP-IO-2500", 					"Solution épaisse à fort pouvoir cosmétique pour la désinfection des trayons en après-traite ● Contient 2500 ppm d'iode, sans gouttage ● A appliquer par trempage sur toute la longueur du trayon|||",																																																																																										"4x5,10,22,60,120,220,1000",	"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"012311,012312,012317,012315,012313,012314,012316",				"AB",	"B",
		"DIP-IO 5000", 								"#Udder Film", 		"4", 															"",			"kg,kg,kg,kg",								"1,Dip-io%205000%20FR.pdf?csf=1&e=V35V5y", 						"DIP-IO-5000", 					"Solution épaisse à fort pouvoir cosmétique pour la désinfection des trayons en après-traite ● Contient 5000 ppm d'iode, ses propriétés rhénologiques lui confèrent une bonne tenue sur le trayon sans gouttage ● A appliquer par trempage sur toute la longueur du trayon ● Idéal pour grand troupeau|||",																																																																	"2,4x5,10,22,60,120,220,1000",	"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"013611,012391,012392,012389,012394,012393,012390,012395",		"AB",	"B",
		"LIQ-IO 2500", 								"#Udder Film", 		"3,7", 															"",			"kg,kg,kg,kg",								"1,Dip-io%202500%20FR.pdf?csf=1&e=jI055R", 						"LIQ-IO-2500", 					"Liq-io 2500 est un désinfectant contenant 2500 ppm d’iode, enrichie en substances adoucissantes, émollientes et hydratantes|||",																																																																																																											"4x5,10,22,60,120,200,1000",	"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"012801,012805,012807,012808,012806,012804,012803",				"AB",	"B",
		"LIQ-IO 5500", 								"#Udder Film", 		"3,7", 															"",			"kg,kg,kg,kg",								"1,Liq-io%205500%20FR.pdf?csf=1&e=zMvZE6", 						"LIQ-IO-5500", 					"Produit spray après-traite à haut pouvoir désinfectant et cosmétique ● Liq-io 5500 contient 5500 ppm d'iode et est enrichi en substances adoucissantes et hydratante pour la peau du trayon ● Produit prêt à l'emploi|||",																																																																																					"200,120,4x5,60,10,1000,22",	"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"012876,012877,012878,012879,012880,012881,012882",				"AB",	"B",
		"LIQ-IO C",									"#Udder des",		"0",															"",			"kg,kg,kg,kg",								"",	 															"LIQ-IO-C",						"Désinfectant à base d'iode, concentré à diluer ● 25 000 ppm d'iode à diluer à 10 % (bactéricidie, levuricidie, algicidie) ou 20 % (pour action virucide supplémentaire) ● Contient différents agents cosmétiques (glycérine et lanoline)|||",																																																																																"4x5,10,20,60,120,220,1000",	"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"012939,012943,012941,012945,012942,012940,012944",				"AB",	"B",
		"HY-COSMETIC",								"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"",	 															"HY-COSMETIC",					"Produit de trempage épais désinfectant et surgraissant à base d'acide lactique, molécule naturelle ● Prend soin de la peau du trayon ● Produit prêt à l'emploi, qui peut s'utiliser tout au long de l'année|||",																																																																																							"2,22,60,220",					"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"014942,014021,015112,016861",									"AB",	"B",
		"FILMADINE",								"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"Filmadine%20FR.pdf?csf=1&e=0QrXwg",	 						"FILMADINE",					"Produit de trempage désinfectant, épais et surgraissant. A base d'acide lactique, molécule naturelle ● Prend soin de la peau du trayon ● Produit prêt à l'emploi, idéal pour les troupeaux à faible risque|||",																																																																																							"10,22,60,200",					"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"003882,003899,003924,008683",									"AB",	"B",
		"HM VIR FILM",								"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"HM%20VIR%20Film%20FR.pdf?csf=1&e=LwddFv",	 					"HM-VIR-FILM",					"Produit de trempage épais cosmétique et désinfectant à base de LSA® ● HM Vir Film contient de l’Aloe Vera reconnu pour ses vertus anti-inflammatoires, cicatrisantes et astringentes ● Le produit est efficace contre les germes responsables des mammites ● Prêt à l'emploi, il peut s'utiliser toute l'année|||",																																																														"2,10,22,60,220,1000",			"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"004180,003502,004124,004135,004130,004140",					"AB",	"B",
		"HM VIR FILM+",								"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"HM%20VIR%20Film%20+%20FR.pdf?csf=1&e=0kzl94",	 				"HM-VIR-FILM+",					"Produit de trempage très marquant à base de LSA®, complexe actif et désinfectant très puissant ● HM Vir Film + contient également de l'Aloe Vera, reconnu pour ses vertus anti-inflammatoires, cicatrisantes et astringeantes ● Le produit vert laisse une très bonne marque sur les trayons ● Prêt à l'emploi|||",																																																														"4x5,10,22,60,120,220,1000",	"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"025807,025809,025806,025812,025810,025811,025808",				"AB",	"B",
		"HM VIR SPRAY",								"#Udder Des",		"3,7",															"420",		"kg,kg,kg,kg",								"",																"HM-VIR-SPRAY",					"Désinfectant à base du complexe LSA®, prêt à l'emploi ● Contient du propylène glycérol|||",																																																																																																																				"10,22,60,220,1000",			"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"003442,003448,003457,003454,003460",							"AB",	"B",
		"HYPRED QUICK SPRAY",						"#Udder des",		"3,7*1.5,3.5*1.64,3.78",										"420",		"kg,kg,kg,kg",								"Hypred%20Quick%20Spray%20fr.pdf?csf=1&e=nDwQ7e",				"HYPRED-QUICK-SPRAY",			"Désinfectant à base du complexe LSA®, prêt à l'emploi ● Désinfection « Flash » des trayons et de la base de la mamelle, agit en 30 secondes ● Enrichi en agents hydratants (propylène glycol, glycérine, allantoïne) et assouplisant ● Utilisable en avant et en après-traite|||",																																																																							"4x5,10,22,60,220,1000",		"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow,goat,sheep",			"002798,002801,002811,002823,002820,002826",					"AB",	"B",
		"DERMIODE",									"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"Hypred%20Quick%20Spray%20fr.pdf?csf=1&e=nDwQ7e",				"DERMIODE",						"Désinfectant à base d'iode, produit de trempage épais prêt à l'emploi ● Ses propriétés rhéologiques lui assure une bonne tenue sur le trayon, sans gouttage et donc sans perte de produit|||",																																																																																												"10,22,60,200",					"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"005426,005427,05428,013950",									"AB",	"B",
		"POWER BLUE MIX ACTIV'",					"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"Power%20Blue%20Mix%20FR.pdf?csf=1&e=uN0i24",					"POWER-BLUE-MIX-ACTIV'",		"Produit de trempage épais, bleu et marquant sur le trayon ● Désinfectant puissant à base de dioxyde de chlore et d'acide lactique, le produit protège aussi les trayons grâce aux agent humectants, emollient et assouplissant présent dans la formule ● S'utilise après mélange 50/50 avec le POWER BLUE MIX Base|||",																																																													"2,10,21,62,228",				"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"016535,016506,016505,016507,017332",							"",		"B",
		"POWER BLUE MIX BASE",						"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"Power%20Blue%20Mix%20FR.pdf?csf=1&e=uN0i24",					"POWER-BLUE-MIX-BASE",			"Produit de trempage épais, bleu et marquant sur le trayon ● Désinfectant puissant à base de dioxyde de chlore et d'acide lactique, le produit protège aussi les trayons grâce aux agent humectants, emollient et assouplissant présent dans la formule ● S'utilise après mélange 50/50 avec le POWER BLUE MIX Activ|||",																																																													"2,10,19,58,212",				"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"016536,016508,016509,016510,017333",							"",		"B",
		"NOVODIP",									"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"",																"NOVODIP",						"Produit de trempage désinfectant utilisable après-traite. Combinaison d'acide lactique et de chlorhexidine pour une action rapide et une protection des trayons. Prêt à l'emploi|||",																																																																																														"22,60,220",					"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"26873,29797,29798",											"AB",	"B",
		"TRAYMIX ACTIV'",							"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"",																"TRAYMIX-ACTIV'",				"Produit de trempage épais, bleu et marquant sur le trayon ● Désinfectant puissant à base de dioxyde de chlore et d'acide lactique, le produit protège aussi les trayons grâce aux agent humectants, emollient et assouplissant présent dans la formule ● S'utilise après mélange 50/50 avec le TRAY MIX BASE|||",																																																															"2,21,62,228",					"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"017703,017701,017704,017702",									"",		"B",
		"TRAYMIX BASE",								"#Udder des",		"3",															"",			"kg,kg,kg,kg",								"",																"TRAYMIX-BASE",					"Produit de trempage épais, bleu et marquant sur le trayon ● Désinfectant puissant à base de dioxyde de chlore et d'acide lactique, le produit protège aussi les trayons grâce aux agent humectants, emollient et assouplissant présent dans la formule ● S'utilise après mélange 50/50 avec le TRAY MIX ACTIV'|||",																																																														"2,19,58,212",					"",			"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"017707,017705,017708,017706",									"",		"B",

		"ANTI-GERM TRAYDOU",						"#Udder Soft",		"3-4",															"",			"kg,kg,kg,kg",								"",																"ANTI-GERM-TRAYDOU",			"Gel désinfectant des trayons en après-traite à base de biphényl-2-ol, prêt à l'emploi ● Ultra protecteur & longue durée ● Bonne visualisation – ne coule pas ● Très riche en agents cosmétiques (Aloe vera, huiles essentielles et vitamine E)||Desinfektionsgel für die Zitzen nach dem Melken auf Basis von Biphenyl-2-ol, gebrauchsfertig ● Hohe und lang anhaltende Schutzwirkung ● Schutz gut sichtbar - tropft nicht ● Reich an kosmetischen Wirkstoffen (Aloe vera, aetherische Öle und Vitamin E)|",																"20,60,200",		"",						"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"",																"",		"B",
		...
		"ANTI-GERM TRAYDIP",						"#Soft",			"3-4",															"",			"kg,kg,kg,kg",								"",																"ANTI-GERM-TRAYDIP",			"Gel désinfectant des trayons en après-traite à base de biphényl-2-ol, prêt à l'emploi ● Bonne visualisation – ne coule pas ● Contient de la glycérine et de l'allantoïne|||",																																																																																																"20,200",			"",						"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"",																"",		"B",
		"ANTI-GERM INTEGRAL", 						"#Udder Film", 		"3-4", 															"",			"kg,kg,kg,kg",								"", 															"ANTI-GERM-INTEGRAL", 			"Désinfectant des trayons en après-traite, filmogène, à base de biphényl-2-ol, prêt à l'emploi ● Film disparaissant avant la traite suivante pour faciliter le nettoyage des trayons ● Riche en agents cosmétiques (glycérine et allantoïne)|||",																																																																															"10,20,60",			"",						"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"",																"",		"B",
		...
		"ANTI-GERM IO-SPRAY", 						"#Udder", 			"3-4", 															"250",		"kg,kg,kg,kg",								"", 															"ANTI-GERM-IO-SPRAY", 			"Désinfectant des trayons à base d'iode (2,7 g/kg), prêt à l'emploi ● Contient des agents protecteurs (glycérine et allantoïne)|||",																																																																																																										"21,60,200",		"",						"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow",						"",																"",		"B",
		...
		"ANTI-GERM PRATIC NF",						"#Udder",			"3-4",															"250",		"kg,kg,kg,kg",								"",																"ANTI-GERM-PRATIC-NF",			"Désinfectant des trayons en avant et après-traite à base de chlorhexidine ● Spécial robots de traite ● Contient des agents cosmétiques, adoucissants et hydratants|||",																																																																																																	"20,60,200",		"",						"kg,kg,kg,kg",									"hygiène mamelle,udder hygiene,,",						"",		"manual",			"",										"",										"cow,goat,sheep",			"",																"",		"B",
		...
		"HYPROCLOR ED",					 			"#Alk", 			"0.6", 															"150",		"kg,kg,kg,kg",								"", 															"HYPROCLOR-ED", 				"Alcalin non chloré détergent, anti-tarte ● Compatible avec les manchons et circuits silicone ● Utilisation en générateur eau bouillante|||",																																																																																																								"25,70,230,1200",	"1.2",					"kg,kg,kg,kg",									"circuits internes,internal circuits,,",				"",		"manual",			"",										"",										"cow",						"004489 023023,004505 023024,4500,004512 023030",				"AB",	"B",
		...
		"AGAKOK 2.5",								"#Des Surf", 		"2.5", 															"10",		"kg,kg,kg,kg",								"", 															"AGAKOK-2.5",					"Désinfectant utilisable à 2,5&nbsp;% contre les cryptosporidies, les coccidies et Ascaris ● Virucide, bactéricide et fongicide à 2 % selon les dernières normes en vigueur EN ● S'utilise par pulvérisation sur les sols et les surfaces propres jusqu'à la limite de ruissellement ● S'applique facilement avec un canon mousse|||",																																																										"10",				"",						"kg,kg,kg,kg",									"dés. des surfaces,surf. disinfection,,",				"",		"manual",			"",										"",										"cow,goat,sheep",			"",																"",		"",
		...
		"LAVETTES microfibres gaufrées",			"#Udder wipes MF",	"1",															"",			"lavettes,wipes,Tücher,paños",				"",																"LAVETTES-MF-GAUFREES",			"Lavettes microfibres d'aspect gaufré ● 32&nbsp;x&nbsp;35 cm ● Ourlets piqués sur 2 côtés ● Composition : polyester et coton ● Poids unitaire : 40&nbsp;g|32 x 35&nbsp;cm|32 x 35&nbsp;cm|32 x 35&nbsp;cm",																																																																																									"10",				"",						"sachet(s) de &Q &P,bag(s) of &Q &P,,,",		"hygiène mamelle,udder hygiene,,",						"",		"auto",				"",										"",										"cow",						"",																"",		"",
		"LAVETTES bouclées éponge",					"#Udder wipes BE",	"1",															"",			"lavettes,wipes,Tücher,paños",				"",																"LAVETTES-BOUCLEES-EPONGE",		"32 x 32&nbsp;cm ● Coton majoritaire et autres fibres|||",																																																																																																																													"10",				"",						"sachet(s) de &Q &P,bag(s) of &Q &P,,,",		"hygiène mamelle,udder hygiene,,",						"",		"auto",				"",										"",										"cow",						"",																"",		"",
		"LAVETTES SUPER",							"#Udder wipes SUP",	"1",															"",			"lavettes,wipes,Tücher,paños",				"",																"LAVETTES-SUPER",				"51 x 36&nbsp;cm ● Gaufrée synthétique||51 x 36 cm ● Synthetische Reinigungstücher|",																																																																																																																						"25",				"",						"sachet(s) de &Q &P,bag(s) of &Q &P,,,",		"hygiène mamelle,udder hygiene,,",						"",		"auto",				"",										"",										"cow",						"",																"",		"",
		...
		"GOBELET MOUSSEUR",							"#Udder cup1",		"1",															"",			",,,",										"",																"GOBELET-MOUSSEUR",				"Démontable ● Plusieurs filtres pour une mousse onctueuse ● 250&nbsp;ml||Zerlegbarer Schaumbecher ● Mehrere Filter für einen weichen Schaum ● 250 ml|",																																																																																																						"1",				"",						",,,",											"hygiène mamelle,udder hygiene,,",						"",		"auto",				"",										"",										"cow,goat",					"",																"",		"",
		"GOBELET BACTO TREMPE",						"#Udder cup2",		"1",															"",			",,,",										"",																"BACTO-TREMPE",					"Anti-retour ● Adapté aux produits visqueux ● 250&nbsp;ml||Dippbecher mit Rücklaufschutz ● Angepasst für die Anwendung von viskosen Produkten ● 250 ml|",																																																																																																					"1",				"",						",,,",											"hygiène mamelle,udder hygiene,,",						"",		"auto",				"",										"",										"cow,goat,sheep",			"",																"",		"",
		"Réservoir gobelets",						"#Udder cup",		"1",															"",			",,,",										"",																"RESERVOIR-GOBELETS",			"Réservoir pour gobelets mousseur/trempeur sans bouchon  ● 250&nbsp;ml|||",																																																																																																																									"1",				"",						",,,",											"hygiène mamelle,udder hygiene,,",						"",		"auto",				"",										"",										"cow,goat,sheep",			"",																"",		"",
		...
		"PULVÉRISATEUR Bovins/Ovins",				"#Udder spray",		"1",															"",			",,,",										"",																"PULVÉRISATEUR-BOVINS-OVINS",	"Spray pour les trayons ● Spécial bovins/ovins|||",																																																																																																																															"1",				"",						",,,",											"hygiène mamelle,udder hygiene,,",						"",		"auto",				"",										"",										"cow,sheep",				"",																"",		"",
		"PULVÉRISATEUR Caprins",					"#Udder spray",		"1",															"",			",,,",										"",																"PULVÉRISATEUR-CAPRINS",		"Spray pour les trayons ● Spécial caprins|||",																																																																																																																																"1",				"",						",,,",											"hygiène mamelle,udder hygiene,,",						"",		"auto",				"",										"",										"goat",						"",																"",		"",
		...
		"BOLIFAST PHYSIO",							"#Nutri",			"2",															"",			"bolus,bolus,Dosen,dosis",					"",																"BOLIFAST-PHYSIO",				"Bolus démarrage de la lactation : soutien nutritionnel des fonctions hépatiques, à base de choline et méthionine rumino protégées. Formule effervescente ● Application : 1 dose 15 jours avant le vêlage|||",																																																																																								"20",				"",						"boîte(s) de &Q &P,boxes of &Q &P,,",			"nutrition,nutrition,,",								"",		"auto",				"VL,cow,kuh,vaca",						"",										"cow",						"006381",														"",		"",
		"BOLITRACE BIOTIN +",						"#Nutri",			"2",															"",			"bolus,bolus,Dosen,dosis",					"",																"BOLITRACE-BIOTIN-+",			"Bolus vaches taries : qualité des phanères, qualité du colostrum et vitalité du veau ● Apports pendant 120 jours (oligo éléments, vitamines A, D3, E, biotine) ●  Application : 2 bolus le jour du tarissement ou 3 mois avant vêlage|||",																																																																																"12",				"",						"boîte(s) de &Q &P,boxes of &Q &P,,",			"nutrition,nutrition,,",								"",		"auto",				"VL,cow,kuh,vaca",						"",										"cow",						"018152",														"",		"",
		...
		"BOLIFLASH FERTIL",							"#Nutri",			"2",															"",			"bolus,bolus,Dosen,dosis",					"",																"BOLIFLASH-FERTIL",				"Bolus fertilité : amélioration de l’expression des chaleurs et prépare à l’IA en une seule application, soutient de la préparation à l’oetrus et à la reproduction ● A base de bêta-carotène ● Application : 1 dose 45 à 80 jours après vêlage|||",																																																																														"20",				"",						"boîte(s) de &Q &P,boxes of &Q &P,,",			"nutrition,nutrition,,",								"",		"auto",				"VL,cow,kuh,vaca",						"",										"cow",						"006297",														"",		"",
		...
		"BOLIDAYS CONTROL",							"#Nutri",			"2",															"",			"bolus,bolus,Dosen,dosis",					"",																"BOLIDAYS-CONTROL",				"Bolus démarrage de lactation : accompagnement du métabolisme énergétique, réduction du risque d'acétonémie, aide à la reproduction ● Libération contrôlée des éléments : apports pendant 10 jours en 1 dose (L-Carnitine, Choline, Méthionine rumino-protégées) ● Application : 2 bolus le 1er jour de la lactation|||",																																																													"12",				"",						"boîte(s) de &Q &P,boxes of &Q &P,,",			"nutrition,nutrition,,",								"",		"auto",				"VL,cow,kuh,vaca",						"",										"cow",						"",																"",		"",	
		"HYDRAFEED",								"#Nutri",			"4",															"",			"sachet(s),bag(s),,",						"",																"HYDRAFEED",					"Formule effervescente d'aide au démarrage du veau  (action protectrice) : transition alimentaire (entre colostrum et phase lactée), aide à la bonne réhydratation en cas de diarrhées légères (blanche) ● Application : transition, 1 journée (2 repas/jr) -réhydratation, 2 à 3 jours (2 repas/jr)|||",																																																																	"16",				"",						"boîte(s) de &Q &P,boxes of &Q &P,,",			"nutrition,nutrition,,",								"",		"auto",				"veau,veal,kalbfleich,ternera",			"",										"cow",						"009105",														"",		"",
		"HYDRAFEED GEL",							"#Nutri",			"1",															"",			"seringue(s),syringe(s),Dosen,dosis",		"",																"HYDRAFEED-GEL",				"Aide au démarrage du veau (action protectrice) ● Aide à la bonne réhydratation sans coupure de la phase lactée ● Application : 1 seringue (en 4 repas sur 2 jours)|||",																																																																																																	"6",				"",						"boîte(s) de &Q &P,boxes of &Q &P,,",			"nutrition,nutrition,,",								"",		"auto",				"veau,veal,kalbfleich,ternera",			"",										"cow",						"011417",														"",		"",
		...
		"POMPE Bidons PX1904",						"#Pump",			"1",															"",			"pompes,pumps,pumps,bombas",				"",																"POMPE-PX1904",					"Pompe doseuse pour bidons de 10 litres ● Dose 30 ml|||",																																																																																																																													"1",				"",						",,,",											"toutes,all,,",											"",		"auto",				"",										"",										"cow",						"",																"",		"",
		"POMPE Bidons PX1910",						"#Pump",			"1",															"",			"pompes,pumps,pumps,bombas",				"",																"POMPE-PX1910",					"Pompe doseuse pour bidons de 20 litres ● Dose 30 ml|||",																																																																																																																													"1",				"",						",,,",											"toutes,all,,",											"",		"auto",				"",										"",										"cow",						"",																"",		"",
		"POMPE PX1948",								"#Pump",			"1",															"",			"pompes,pumps,pumps,bombas",				"",																"POMPE-PX1948",					"Pompe doseuse pour fûts de 60 litres ● Dose 30 ml|||",																																																																																																																														"1",				"",						",,,",											"toutes,all,,",											"",		"auto",				"",										"",										"cow",						"",																"",		"",
		"POMPE EP93/100 PX1911",					"#Pump",			"1",															"",			"pompes,pumps,pumps,bombas",				"",																"POMPE-PX1911",					"Pompe pour fûts de 60 litres ● Dose 100 ml|||",																																																																																																																															"1",				"",						",,,",											"toutes,all,,",											"",		"auto",				"",										"",										"cow",						"",																"",		"",
		"POMPE EP93/100 PX1939",					"#Pump",			"1",															"",			"pompes,pumps,pumps,bombas",				"",																"POMPE-PX1939",					"Pompe pour fûts de de 200 litres ● Dose 100 ml|||",																																																																																																																														"1",				"",						",,,",											"toutes,all,,",											"",		"auto",				"",										"",										"cow",						"",																"",		"",
		"POMPE VF CONTI PX1940",					"#Pump",			"1",															"",			"pompes,pumps,pumps,bombas",				"",																"POMPE-PX1940",					"Pompe de transfert|||",																																																																																																																																					"1",				"",						",,,",											"toutes,all,,",											"",		"auto",				"",										"",										"cow",						"",																"",		"",
		"POMPE EZI-ACTION PX1951",					"#Pump",			"1",															"",			"pompes,pumps,pumps,bombas",				"",																"POMPE-PX1951",					"Pompe gros débit avec sécurité enfants|||",																																																																																																																																"1",				"",						",,,",											"toutes,all,,",											"",		"auto",				"",										"",										"cow",						"",																"",		""
	];
